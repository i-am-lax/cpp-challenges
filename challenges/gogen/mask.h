...
class Mask {
  ...
  public:
    Mask(bool value = true) { set_all(value); }
    ...
    Mask operator+(const Mask &other) { return binary_operation(other, op_or); }
    ...
    Mask operator!() const { return unary_operation(op_not); }

    bool *operator[](int n) { return mask[n]; }

  private:
    static bool op_and(bool one, bool two) { return one && two; }

    static bool op_subtract(bool one, bool two) { return one && !two; }

    static bool op_or(bool one, bool two) { return one || two; }

    static bool op_not(bool input) { return !input; }

    static int bool_to_int(bool input) { return (input) ? 1 : 0; }
    ...
    void set_region(int tl_row, int tl_col, int br_row, int br_col,
                    bool value) {
